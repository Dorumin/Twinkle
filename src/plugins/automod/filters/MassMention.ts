...
const MassMentionConfigSchema = t.type({
    MASS_MENTION: t.type({
        MINMENTIONS: t.number,
        SPANSECONDS: t.number
    })
});

export default class MassMentionFilter extends Filter {
    private config: t.TypeOf<typeof MassMentionConfigSchema>['MASS_MENTION'];
    private cache: Cache<string, number>;

    constructor(automod: AutomodPlugin, config: ConfigProvider) {
        super(automod, config);

        this.config = config.getOptionTyped('AUTOMOD', MassMentionConfigSchema)['MASS_MENTION'];

        this.cache = new Map();
    }

    suppressMentions(text: string) {
    ...
    interested(message: Message) {
        ...
        }, this.config.SPANSECONDS * 1000);

        if (this.cache.get(message.author.id) < this.config.MINMENTIONS) return false;

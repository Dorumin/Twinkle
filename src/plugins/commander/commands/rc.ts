...
export default class RCCommand extends Command {
    ...
    async call(message: Message, content: string) {
        const link = content
            ? `<${await this.fandomizer.url(content)}/wiki/Special:RecentChanges?feed=rss>`
            : this.fmt.code('https://<yourwiki>.fandom.com/wiki/Special:RecentChanges?feed=rss');
